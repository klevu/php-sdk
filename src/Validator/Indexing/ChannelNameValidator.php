...
class ChannelNameValidator implements ValidatorInterface
{
    /**
     * [a-zA-Z0-9_]+: Matches one or more alphanumeric characters or underscores.
     */
    private const CHANNEL_NAME_REGEX = '/^[a-zA-Z0-9_]+$/';

    /**
     * Validates that the passed data is a string in a valid format to use as an indexing channel name
     *
    ...
    private function validateType(mixed $data): void
    {
        ...
        if (null !== $data && !is_string($data)) {
            throw new InvalidTypeValidationException(
                errors: [
                    sprintf(
                        'Channel Name must be string, received %s',
                        get_debug_type($data),
                    ),
                ],
            );
    ...
    {
        if (!preg_match(self::CHANNEL_NAME_REGEX, $data)) {
            throw new InvalidDataValidationException(
                errors: [
                    'Channel Name must be alphanumeric, and can include underscores (_)',
                ],
            );
        }
    }
